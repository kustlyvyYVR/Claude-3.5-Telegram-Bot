...
async def change_system_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    mode, session = get_session(update, context)

    if mode == "Bard":
        return await update.message.reply_text("❌ Invalid option for Google Bard.")
    if len(context.args) < 1:
        return await update.message.reply_text("❌ Please provide a system prompt.")

    system_prompt = " ".join(context.args)
    session.system_prompt = system_prompt
    await update.message.reply_text(
        f"🗣️ System prompt has been set to:\n<b>{system_prompt}</b>",
...
async def recv_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "Claude":
        response_text = await session.send_message(input_text)

        if not response_text.strip():
            await message.edit_text("❌ Empty response received. Please try again.")
            return
        ...
        try:
            ...
            if str(e).startswith("Message is not modified"):
                ...
                await message.edit_text(f"{response_text.strip()[:4095]}.")
            else:
                print(f"[e] {e}")
                await message.edit_text(f"❌ Error occurred: {e}. /reset")
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "Claude":
        extras = [
            ...
            f"System Prompt: <b>{session.system_prompt}</b>",
            "",
            "Available Tasks:",
            ", ".join(session.get_available_tasks()),
            "",
            "Commands:",
            ...
            "• [/system PROMPT] to set system prompt",
            "• [/task NAME] to select a predefined task",
...
async def change_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "Bard":
        return await update.message.reply_text("❌ Invalid option for Google Bard.")
    if len(context.args) != 1:
        return await update.message.reply_text("❌ Please provide a task name.")

    task = context.args[0].strip()
    if not session.change_system_prompt(task):
        return await update.message.reply_text("❌ Invalid task name.")
    await update.message.reply_text(
        f"📝 Task has been set to <b>{task}</b>.", parse_mode=ParseMode.HTML
    )
...
def run_bot():
    ...
    handler_list = [
        ...
        CommandHandler("system", change_system_prompt, user_filter),
        CommandHandler("task", change_task, user_filter),
